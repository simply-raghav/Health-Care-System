const patient_data = (id) => {
  fetch("/authDoctor/users", {
    ...
    .then((data) => {
        ...
        featureTimings(data.result.slot_interval, data.result.timings);
    });
};
...
const featureTimings = (slot_interval, timings) => {
  ...
  const generateDates = () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      li.addEventListener("click", function () {
        ...
        if (day === "Sun") {
          generateTimeSlots(
            parseInt(slot_interval),
            timings.Sunday.startTime.toString(),
            timings.Sunday.endTime.toString()
          );
        } else if (day === "Mon") {
          generateTimeSlots(
            parseInt(slot_interval),
            timings.Monday.startTime,
            timings.Monday.endTime
          );
        } else if (day === "Tue") {
          generateTimeSlots(
            parseInt(slot_interval),
            timings.Tuesday.startTime.toString(),
            timings.Tuesday.endTime.toString(),
          );
        } else if (day === "Wed") {
          generateTimeSlots(
            parseInt(slot_interval),
            timings.Wednesday.startTime,
            timings.Wednesday.endTime
          );
        } else if (day === "Thu") {
          generateTimeSlots(
            parseInt(slot_interval),
            timings.Thursday.startTime,
            timings.Thursday.endTime
          );
        } else if (day === "Fri") {
          generateTimeSlots(
            parseInt(slot_interval),
            timings.Friday.startTime,
            timings.Friday.endTime
          );
        } else if (day === "Sat") {
          generateTimeSlots(
            parseInt(slot_interval),
            timings.Saturday.startTime,
            timings.Saturday.endTime
          );
        }
      });
  ...
  // Function to generate time slots dynamically based on parameters
  const generateTimeSlots = (slotDuration, startTime, endTime) => {
